...
int TUI::renderMainWin(std::vector<std::string> tasks)
{
    ...
    while (true)
    {
        ...
        case KEY_PPAGE: // Page Up
            main_window_highlight = std::max(0, main_window_highlight - visible_height);
            break;
        case KEY_NPAGE: // Page Down
            main_window_highlight = std::min((int)tasks.size() - 1, main_window_highlight + visible_height);
            break;
        default:
            break;
        }
